	...
	<view class="container">
		<view class="header-section">
			<view class="nav-bar">
				...
				<text class="page-title">个人信息修改</text>
			</view>
			...
			<view class="glass-card animate-fade-up">
				...
				<view class="form-group">
					...
					<view class="form-item">
						...
						<input class="input" type="idcard" v-model="formData.idCardNumber" placeholder="请输入身份证号码" maxlength="18" placeholder-style="color:#BCBCBC"/>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			// 如果有查询原有信息并回显的逻辑，请写在这里
			this.fetchUserInformation();
		},
		methods:{
			async fetchUserInformation() {
				// 模拟回显：实际中应调用获取信息的接口
				// let res = await uni.$http.get('/loan/information');
			...
			async submitForm() {
				// 修改提交接口
				let res = await uni.$http.post('/loan/information/modify', this.formData);
				if(res.code == 200){ 
